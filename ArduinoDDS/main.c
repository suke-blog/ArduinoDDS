//*****************************************************************************
// This code is Modified from "AVR DDS2 signal generator" by suke-blog.com.
// TargetMCU changed ATmega16 to ATmega168/328(Arduino).
...
static const uint8_t squarewave[] __attribute__((aligned(256))) PROGMEM = //square wave
{
...
static const uint8_t sawtoothwave[] __attribute__((aligned(256))) PROGMEM = //sawtooth wave
{
...
static const uint8_t rewsawtoothwave[] __attribute__((aligned(256))) PROGMEM = //reverse sawtooth wave
{
...
static const uint8_t trianglewave[] __attribute__((aligned(256))) PROGMEM = //triangle wave
{
...
static const uint8_t ECG[] __attribute__((aligned(256))) PROGMEM = //ECG wave
{
...
void Timer2_Stop(void)
{
	TCCR2B&=~((1<<CS22)|(1<<CS21)); //Stop timer0
	TIMSK2&=~(1<<TOIE2);//Disable Timer0 Overflow interrupts
}
...
{
	debug_write("[DEBUG] Entry Menu_Update.\n");
...
//Pin Change Interrupt0 service routine
...
ISR(PCINT0_vect)
{
// debug_write("[DEBUG] Entry ISR(PCINT0_vect)\n");

SG.flag=0;//set flag to stop generator
// SPCR|=(1<<CPHA);//using CPHA bit as stop mark
GPIOR0|=(1<<2);//using GPIOR0 as stop mark
//CopyStringtoLCD(MNOFF, 13, 1 );
...
ISR(TIMER2_OVF_vect)
{
	// debug_write("[DEBUG] Entry ISR(TIMER2_OVF_vect)\n");
...
if(SG.mode!=6)
	{
		...
		GPIOR0 = 0;//clear GPIOR0 register to allow DDS
...
change SPCR register to GPIOR0 for ATmega168/328(Arduino).
*/
...
//enable external interrupt for start/stop button(PCINT10)
void Pcint_Enable(){
	PCIFR = 0;	//flag clear
	PCMSK0 |= (1<<PCINT5);
	PCICR  |= (1<<PCIE0);
...
//disable external interrupt for start/stop button(PCINT10)
void Pcint_Disable(){
	PCMSK0 &= ~(1<<PCINT5);
	PCICR  &= ~(1<<PCIE0);
}
...
void Main_Init(void)
{
	debug_write("[DEBUG] Entry Main_Init.\n");
...
{
	debug_write("[DEBUG] Entry avrdds_main.\n");
...
while(1)//infinite loop
	...
	if (SG.flag==1)
		{
		Pcint_Enable();//set external interrupt to enable stop
		if (SG.mode==7)
			{
			//Noise
			do
...
/*		else if (SG.mode==6)
			...
			//display generator OFF
			Menu_Update(SG.ON);
			Pcint_Disable();//stop external interrupt
			//start timer menu active
			Timer2_Start();
			}
		else
